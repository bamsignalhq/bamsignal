...
export type ConsultationPaymentPhase = "idle" | "preparing" | "checkout" | "verifying" | "paid" | "pending" | "failed";
...
async function postConsultationPayment(
  ...
  const response = await fetch(apiUrl(`/api/consultation-payment?action=${action}`), {
    method: "POST",
...
export function deriveConsultationPaymentPhase(
  ...
  if (flow === "verifying" || payment.status === "initialized") return flow === "verifying" ? "verifying" : "pending";
...
export async function verifyConsultationPayment(
  ...
  setPaymentFlowState("verifying");
  const response = await fetch(apiUrl("/api/consultation-payment?action=verify"), {
    method: "POST",
    headers: await memberApiHeaders(),
    body: JSON.stringify({
  ...
  const payload = (await readResponseJson<VerifyPayload>(response)) ?? {};
