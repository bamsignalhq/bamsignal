-- Sprint 2 — Authentication & Account Lifecycle (operational layer).
-- Extends Supabase Auth; does not replace it. Append-only audit; no hard deletes.
...
-- Member session registry (metadata — Supabase remains token authority)
...
-- Account lifecycle transition log (append-only)
...
-- Recovery tokens (PIN reset, forgot username, lost device)
...
-- Legal retention metadata for deleted accounts
