...
export function registerExistingJourneyId(
...
export function assignJourneyId(
  state: JourneyRegistryState,
  input: { memberId: string; createdAt?: string; now?: string }
): { state: JourneyRegistryState; journeyId: string; created: boolean } {
...
export function getJourneyIdForMember(state: JourneyRegistryState, memberId: string): string | null {
  return state.memberIndex[memberId] ?? null;
}

export function getMemberIdForJourney(state: JourneyRegistryState, journeyId: string): string | null {
  const normalized = normalizeJourneyId(journeyId);
  return state.entries[normalized]?.memberId ?? null;
}

export function listJourneyRegistryEntries(state: JourneyRegistryState): JourneyRegistryEntry[] {
