...
/** Camera roll / messenger exports — platform words in the basename are not contact leaks. */
...
/**
 * Filename-only safety — ignores messenger/camera auto-names for platform-word rules.
...
export function scanPhotoFilenameSafetyText(text, { allowDocuments = false } = {}) {
  ...
  if (isAutoGeneratedPhotoFilename(raw)) {
    if (containsExplicitContactUrl(raw)) {
      return { blocked: true, category: "contact_information" };
    }
    return { blocked: false, category: null };
  }

  return scanPhotoSafetyText(raw, { allowDocuments });
}
